...
type ViewType = 'landing' | 'goals' | 'loading' | 'roadmap' | 'dashboard' | 'chat';
...
function App() {
  ...
  const handleUpdateProgress = (pathId: string, updatedPath: LearningPath) => {
    setSavedPaths(prev => prev.map(p => p.id === pathId ? updatedPath : p));
  };
      ...
      phases: [
        {
          ...
          resources: [
            { title: 'Getting Started Guide', type: 'documentation', url: 'https://example.com' },
            { title: 'Introduction Course', type: 'course', url: 'https://example.com' }
          ],
  ...
  switch (currentView) {
    case 'landing':
      return (
        <>
          <Navbar currentView={currentView} onNavigate={handleNavigate} savedPathsCount={savedPaths.length} />
          <LandingPage onGetStarted={handleGetStarted} onStartChat={handleStartChat} />
        </>
      );
