...
export function GoalSelection({ onBack, onGoalSelect }: GoalSelectionProps) {
  ...
  const categories = ['All', ...Array.from(new Set(careerGoals.map(goal => goal.category)))];
        ...
        <div className="mb-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Select the career path you want to pursue, and we'll generate a personalized learning roadmap just for you.
            </p>
          </div>
        </div>
...
function GoalCard({ goal, onClick }: { goal: CareerGoal; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="group p-6 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer border border-white/20 hover:scale-105 hover:bg-white"
    >
