...
export function ChatInterface({ onBack, onRoadmapGenerated }: ChatInterfaceProps) {
...
What would you like to learn today?`,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const isRoadmapRequest = /(?:become|learn|roadmap|path|career|want to be|how to be).*(?:developer|engineer|analyst|scientist|designer|manager)/i.test(inputValue);
      
      if (isRoadmapRequest) {
        // Extract career goal
        const careerMatch = inputValue.match(/(?:become|be|learn|roadmap for|path for)\s+(?:a\s+)?([^.!?]+)/i);
        const careerGoal = careerMatch ? careerMatch[1].trim() : inputValue;

        const loadingMessage: Message = {
          id: (Date.now() + 1).toString(),
          type: 'bot',
          content: '🤖 **Generating your personalized learning roadmap...**\n\nThis might take a moment as I analyze the best learning path for you!',
          timestamp: new Date(),
...
Click the button below to view your complete roadmap with resources and projects!`,
          ...
          setMessages(prev => prev.filter(msg => !msg.isGeneratingRoadmap).concat([successMessage]));
          ...
          const errorMessage: Message = {
            id: (Date.now() + 2).toString(),
            type: 'bot',
            content: '❌ **Sorry, I encountered an error generating your roadmap.**\n\nPlease try rephrasing your request or try again in a moment. You can ask something like:\n- "I want to become a Frontend Developer"\n- "Create a roadmap for Data Science"',
            timestamp: new Date()
          };

          setMessages(prev => prev.filter(msg => !msg.isGeneratingRoadmap).concat([errorMessage]));
        }
      } else {
